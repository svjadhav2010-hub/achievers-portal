  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--surface-2)', fontFamily: 'var(--font-sans)' }}>
      ...
      <nav className="nav-bar">
        <div className="nav-inner">
          <Link href="/" style={{ display:'flex', alignItems:'center', gap:10, textDecoration:'none' }}>
            ...
            <span className="serif" style={{ fontSize:16, color:'var(--text-primary)', letterSpacing:'-0.01em' }}>Achievers Portal</span>
          </Link>
      ...
      <main style={{ maxWidth:1100, margin:'0 auto', padding:'36px 24px' }}>
        {loading && <div style={{ textAlign:'center', padding:'80px 0', color:'var(--text-faint)', fontSize:14 }}>Loading your dashboard...</div>}
        ...
        {!loading && data && (<>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:16, marginBottom:32 }}>
            <div style={{ display:'flex', alignItems:'center', gap:14 }}>
              ...
              <div>
                <h1 className="serif" style={{ fontSize:26, letterSpacing:'-0.02em', color:'var(--text-primary)', marginBottom:3 }}>Welcome back, {data.user.fullName.split(' ')[0]}!</h1>
          ...
          <div className="stats-grid" style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:14, marginBottom:28 }}>
            {[
              ...
              <div key={i} className="stat-card" style={{ borderTop:`3px solid ${s.color}` }}>
                <div style={{ fontSize:11, fontWeight:700, color:'var(--text-faint)', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:8 }}>{s.label}</div>
                <div className="serif" style={{ fontSize:30, letterSpacing:'-0.02em', color:s.color, marginBottom:4 }}>{s.val}</div>
                <div style={{ fontSize:12, color:'var(--text-faint)' }}>{s.sub}</div>
              </div>
            ))}
          </div>
          ...
          <div className="dashboard-grid" style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:20 }}>
              ...
              {activeTab === 'tasks' && (
                <div style={{ background:'var(--card-bg)', border:'1px solid var(--border)', borderRadius:20, overflow:'hidden' }}>
                  ...
                  <div style={{ padding:'18px 20px', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <div>
                      <div style={{ fontSize:15, fontWeight:700, color:'var(--text-primary)', marginBottom:2 }}>My Tasks</div>
                      <div style={{ fontSize:12, color:'var(--text-faint)' }}>Your tasks + tasks assigned by your mentor</div>
                    </div>
                    <div style={{ display:'flex', gap:6 }}>
                      {doneTasks > 0 && <span style={{ fontSize:11, fontWeight:700, color:'#8dc63f', background:'rgba(141,198,63,0.12)', borderRadius:100, padding:'3px 10px' }}>{doneTasks} done</span>}
                  ...
                  <div style={{ padding:'12px 16px', borderBottom:'1px solid rgba(0,0,0,0.05)', display:'flex', gap:8, flexWrap:'wrap' }}>
                    ...
                    <input type="date" value={newDueDate} onChange={e => setNewDueDate(e.target.value)}
                      style={{ padding:'9px 10px', border:'1px solid rgba(0,0,0,0.1)', borderRadius:10, fontSize:13, fontFamily:'var(--font-sans)', outline:'none', color:'var(--text-secondary)' }} />
                  ...
                  <div>
                    ...
                    {['pending', 'in_progress', 'completed'].map(statusGroup => {
                      ...
                      return (
                        <div key={statusGroup}>
                          <div style={{ padding:'8px 20px 4px', fontSize:10, fontWeight:700, color:'var(--text-faint)', textTransform:'uppercase', letterSpacing:'0.08em', background:'var(--paper)' }}>
                            ...
                            return (
                              <div key={task.id} className="task-row">
                                ...
                                <button onClick={() => deleteTask(task.id)}
                                  style={{ background:'none', border:'none', cursor:'pointer', color:'var(--border-strong)', fontSize:14, flexShrink:0, padding:'4px 6px', borderRadius:6, transition:'all 0.15s' }}
              ...
              {activeTab === 'modules' && (
                <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                  ...
                  <div className="stat-card" style={{ display:'flex', alignItems:'center', gap:16 }}>
                    <div style={{ fontSize:13, fontWeight:600, color:'var(--text-secondary)', flexShrink:0 }}>Overall</div>
                    <div className="progress-bar-bg">
                  ...
                  {MODULES.map(mod => (
                    <div key={mod.id} className={`module-row ${mod.status === 'locked' ? 'locked' : ''}`}>
                      <div style={{ width:36, height:36, borderRadius:10, background: mod.status === 'locked' ? 'var(--surface-3)' : mod.status === 'completed' ? '#f2f9e6' : 'var(--teal-light)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        <div style={{ fontSize:12, color:'var(--text-faint)', lineHeight:1.4 }}>{mod.desc}</div>
                      ...
                      <span style={{ fontSize:10, fontWeight:700, padding:'4px 10px', borderRadius:100, textTransform:'uppercase', letterSpacing:'0.05em', flexShrink:0,
                        background: mod.status === 'completed' ? 'rgba(141,198,63,0.12)' : mod.status === 'in-progress' ? 'var(--teal-light)' : '#f5f5f5',
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
              ...
              {/* Daily session */}
              <div style={{ background:'#0a0a12', borderRadius:20, padding:'22px 24px', position:'relative', overflow:'hidden' }}>
